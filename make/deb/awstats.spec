    ...
    configure)
        ...
        if [ ! -f "$DBIP_DEST" ]; then
            ...
            if command -v wget > /dev/null 2>&1; then
                wget -q -O "$DBIP_TEMP_GZ" "$DBIP_URL" && \
        ...
        # 设置每月自动更新脚本
        cat > /etc/cron.monthly/update-dbip << 'INNEREOF'
#!/bin/bash
# ------------------------------------------------------------------------------
